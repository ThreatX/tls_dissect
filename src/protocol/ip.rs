...
IPv4 {{ 
    ...
    ip_src: {:?},
    ip_dst: {:?}
}}", self.version, self.header_len, self.ip_tos, self.ip_len, self.ip_id, self.ip_off, self.ip_ttl, self.ip_p, self.ip_sum, self.ip_src, self.ip_dst)
    }
}
...
impl IPv4 {
    pub fn process(&mut self, data:  &[u8]) -> Result<bool, Error> {
        ...
        self.ip_tos = data[1];
        self.ip_len = try!(conversion::buffer_to_uint::<u16>(&data[2..4]));
        self.ip_id = try!(conversion::buffer_to_uint::<u16>(&data[4..6]));
        self.ip_off = try!(conversion::buffer_to_uint::<u16>(&data[6..8]));
        self.ip_ttl = data[8];
        self.ip_p = data[9];
        self.ip_sum = try!(conversion::buffer_to_uint::<u16>(&data[10..12]));
