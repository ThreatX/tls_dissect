...
impl TCP {
    pub fn process(&mut self, data: &[u8]) -> Result<bool, Error> {
        self.s_port = conversion::buffer_to_uint::<u16>(&data[0..2])?;
        self.d_port = conversion::buffer_to_uint::<u16>(&data[2..4])?;
        self.seq_nr = conversion::buffer_to_uint::<u32>(&data[4..8])?;
        self.ack_nr = conversion::buffer_to_uint::<u32>(&data[8..12])?;
        self.data_off = (data[12] & 0xf0) >> 4;
        self.flags = data[13];
        self.window = conversion::buffer_to_uint::<u16>(&data[14..16])?;
        self.sum = conversion::buffer_to_uint::<u16>(&data[16..18])?;
        self.urp = conversion::buffer_to_uint::<u16>(&data[18..20])?;
